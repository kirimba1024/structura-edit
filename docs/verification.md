...
Прогоны исходной версии 0.1.0a2 прошли всю матрицу: core/render, установленный edit
и workspace. Каждый job установленного редактора прошёл 872 теста и пропустил два
теста иконок без client assets; локально прошли все 874. Точные commits, ссылки
...
Тест repeat-панели задаёт минимальную ширину тестового окна явно и проверяет
фактический размер: оконная система CI может уменьшить первоначальный `resize`
до размеров экрана. Запрошенная, первоначальная и доступная ширина сохраняются
в свойствах JUnit. В Windows runner доступно 1024 px, оба окна первоначально
