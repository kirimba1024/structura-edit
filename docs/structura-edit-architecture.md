...
render — моделями, meshing, проекциями и кубической геометрией LOD. Engine/model не импортируют GUI.
...
Recipe использует тот же History с временной SQLite и ограниченным кэшем; весь Undo/Redo
внутри скрипта сохраняется. Каталог принадлежит родительскому Worker: обычное завершение
и исключение удаляют журнал, Cancel удаляет каталог после остановки процесса.
...
Преобразование координат рассчитывается один раз на проекцию в кадре. `map_picking`
использует ту же проекцию/slab/Cave; иконка под курсором получает приоритет над блоком.
`preview_orbit` управляет плавным вращением по времени и ручным drag; наведение
приостанавливает автоматическое вращение.
