...
class ObjectInspector(QDialog):
    ...
    def __init__(self, parent, records, *, readonly=False):
        ...
        self.search.setAccessibleName("Find in object")
        ...
        row = QHBoxLayout()
        row.setSpacing(GRID)
        row.addWidget(CellLabel("Read only" if readonly else "Undo available after Apply"), 1)
        self.apply = QPushButton("Apply changes")
        self.apply.setEnabled(not readonly)
        self.apply.clicked.connect(self.submit)
        close = QPushButton("Close" if readonly else "Cancel")
