...
def prepare_clipboard(session, *, selection=None, clipboard=None, path=None, source_options=None, assets=None,
                      render=True, scene_bytes=0, transform=None, progress=None, take=False, height=HeightSlice()):
    if path is not None:
    ...
    geometry = prepare_geometry(build_geometry(clipboard.render_source(include_entities=False), assets)) if render else None
    if render and clipboard.entities:
        source = clipboard.render_source(include_blocks=False)
        source.size = (1, 1, 1)
        geometry["entities"] = prepare_geometry(build_geometry(source, assets))
        geometry["geometry_bytes"] += geometry["entities"]["geometry_bytes"]
    if render and take and clipboard.cells:
        removed = session.erase(selection)
        prepared = prepare_sections(session, removed, previous=(session, None), height=height, previous_height=height)
        geometry["source_cut"] = build_sections(**prepared, assets=assets, progress=progress)
        geometry["geometry_bytes"] += geometry["source_cut"]["geometry_bytes"]
