...
def load_distance():
    try:
        ...
        if type(radius) is int and type(vertical) is int and 0 <= radius <= MAX_RADIUS and 16 <= vertical <= 192:
...
def save_distance(radius, vertical):
    ...
    atomic_write(root / "world-view.json", json.dumps(dict(radius=radius, vertical_radius=vertical)).encode())
