...
Локальная сборка для проверок на Apple Silicon из текущих исходников workspace. Приложение
...
## Ограничения локальной сборки

В GitHub публикуются исходники; `.app`, ZIP приложения и бинарные пакеты остаются
локальными результатами проверок и не прикладываются к релизу. Приложение имеет
ad hoc подпись. Developer ID и notarization не настроены; перенос сборки на другой
Mac требует отдельной проверки Gatekeeper.
