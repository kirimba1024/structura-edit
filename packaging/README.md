...
Это локальная alpha-сборка с ad hoc подписью. Developer ID и notarization не
настроены: пакет ещё не подготовлен для публичного распространения через
Gatekeeper. Проверка `codesign --verify` не заменяет notarization.
